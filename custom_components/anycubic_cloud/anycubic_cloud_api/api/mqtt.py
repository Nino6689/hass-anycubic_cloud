...
# paho-mqtt 2.x requires CallbackAPIVersion as the first argument to Client().
# We pin VERSION1 so the existing callback signatures (on_connect with rc,
# on_disconnect with rc, on_message with msg, on_subscribe with mid/granted_qos)
# keep working without rewriting every callback in the file.
# Fall back to a sentinel for paho-mqtt 1.x where this enum doesn't exist.
try:
...
class AnycubicMQTTAPI(AnycubicAPIFunctions):
    ...
    def connect_mqtt(self) -> None:
        ...
        # paho-mqtt 2.x: pass callback_api_version positionally; 1.x: omit.
        if _MQTT_CALLBACK_API_VERSION is not None:
            self._mqtt_client = mqtt_client.Client(
                _MQTT_CALLBACK_API_VERSION,
